...
if (count($winners)) {
    foreach ($winners as $winner) {
        mysqli_query($con, 'UPDATE lots SET winner_id = ' . $winner['user_id'] . ' WHERE id = ' . $winner['lot_id']);
        $sql = 'SELECT lots.id, lots.lot_name AS lot_name, users.email, users.name AS user_name FROM lots '
